...
curl -XPUT --fail "$cluster_url/_cluster/settings" -d '{
  "persistent": {
...
curl -XPOST  --fail "$cluster_url/_snapshot/$es_repo_name" -d "{
  \"type\": \"s3\",
    \"settings\": {
      \"bucket\": \"$s3_bucket\",
      \"readonly\": $read_only,
...
## 3. import snapshot
curl -XPOST --fail "$cluster_url/_snapshot/$es_repo_name/$snapshot_name/_restore" -d "{
  \"indices\": \"pelias\",
...
if [[ "$alias_name" != "" ]]; then
  ...
  curl -XPOST --fail "$cluster_url/_aliases" -d "{
    \"actions\": [{
      \"add\": {
        \"index\": \"$snapshot_name\",
        \"alias\": \"$alias_name\"
...
curl -XPUT --fail "$cluster_url/$snapshot_name/_settings" -d "{
  \"index\" : {
